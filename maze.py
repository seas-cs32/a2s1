'''Maze data structure for CS 32 Act II, Scene I'''
...
class Cell(object):
    ...
    def __init__(self, config):
        ...
        self.north = (walls & 0x8) != 0
        self.east = (walls & 0x4) != 0
        self.south = (walls & 0x2) != 0
        self.west = (walls & 0x1) != 0
        self.content = ' '
...
class Maze(object):
    ...
    def __init_row(self, cells):
        ...
        row[0].east = row[1].west
        row[self.width + 1].west = row[self.width].east
    ...
    def __init__(self, cells, endpts):
        ...
        for i in range(self.width + 1):
            self.grid[0][i].south = self.grid[1][i].north
            self.grid[self.height+1][i].north = self.grid[self.height][i].south
    ...
    def move(self, location, direction):
        ...
        if m == 'n' and not self.grid[row][col].north:
            row -= 1
        if m == 's' and not self.grid[row][col].south:
            row += 1
        if m == 'e' and not self.grid[row][col].east:
            col += 1
        if m == 'w' and not self.grid[row][col].west:
            col -= 1
